...
contract DetectionBot is IDetectionBot {
    ...
    function handleTransaction(address player, bytes calldata msgData) external override {
...
contract DoubleEntryPointTest is Test {
    ...
    function setUp() public {
        ...
        instance = new DoubleEntryPoint(address(legacyToken), address(cryptoVault), address(forta), address(this));
    ...
    function test() public {
        DetectionBot detectionBot = new DetectionBot(address(forta), address(this));

        instance.forta().setDetectionBot(address(detectionBot));
        assertEq(address(instance.forta().usersDetectionBots(address(this))), address(detectionBot));
